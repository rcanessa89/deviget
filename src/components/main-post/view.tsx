...
const shouldReRender = (prev: MainPostProps, props: MainPostProps): boolean => {
  if (prev.title !== props.title && prev.author !== props.author) {
    return false;
  }

  const prevImgUrl =
    prev.preview?.images && prev.preview?.images.length
      ? prev.preview.images[0].source.url
      : null;
  const imgUrl =
    props.preview?.images && props.preview?.images.length
      ? props.preview.images[0].source.url
      : null;
    ...
    return (
      <>
        ...
        <Dialog
          ...
          aria-labelledby={`Image of ${title}`}
          open={isImgOpen}
        >
          {imgUrl && (
            <Card
